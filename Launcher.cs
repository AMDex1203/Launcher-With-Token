    ...
    public partial class Launcher : Form
    {
        ...
        private void Form_Closing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    DialogResult Result = MessageBox.Show("Want to close the program ?", Modul.Name, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                ...
                if (Button == 1)
                {
                    if (File.Exists(Application.StartupPath))
                    {
                        ...
                        {
                            Value = true;
                            MessageBox.Show("Unable to start game because there is no file PointBlank.exe", Modul.Name, MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
                        }
                    }
                    else
                    {
        ...
        private void btn_Login_Click(object sender, EventArgs e)
        {
            try
            {
                //if (GetFileSize("helperx.dll") == 77824)
                    ...
                    if (PerformRequest("http://launcher.pb-memories.id/pblauncher/launcher/check.php", txt_User.Text, txt_Pass.Text) == "ชื่อผู้ใช้งาน หรือ รหัสผ่านไม่ถูกต้อง")
                    {
                        MessageBox.Show(PerformRequest("http://launcher.pb-memories.id/pblauncher/launcher/check.php", txt_User.Text, txt_Pass.Text));
                        txt_Pass.Text = "";
                        txt_User.Text = "";
                        txt_User.Focus();
                    }
                    else if (PerformRequest("http://launcher.pb-memories.id/pblauncher/launcher/check.php", txt_User.Text, txt_Pass.Text) == "ban")
                    {
                    ...
                    else
                    {
                        ...
                        Player resultPlayer = JsonConvert.DeserializeObject<Player>(PerformRequest("http://launcher.pb-memories.id/pblauncher/launcher/check.php", txt_User.Text, txt_Pass.Text));
            ...
            {
                MessageBox.Show("You can use Newtonsoft.Json.dll to work together with Newtonsoft.Json.dll", "Warn");
            }
        }
